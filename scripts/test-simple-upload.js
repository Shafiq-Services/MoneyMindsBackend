...
const TEST_FILE_PATH = path.join(__dirname, '../temp/test-simple-upload.bin');
const TEST_FILE_NAME = `test-simple-upload-${Date.now()}.bin`;
...
function progressCallback(progress) {
  ...
  console.log(`[${timestamp}] 📊 Progress: ${progress.progress}% | Speed: ${progress.uploadSpeed} | ETA: ${progress.timeRemaining} | ${progress.message}`);
}
...
async function testSimpleUpload() {
  try {
    ...
    if (error.message.includes('B2_BUCKET_ID not configured')) {
      console.log('💡 Solution: Set your B2 credentials in environment variables');
